...
from asyncio import Event, get_event_loop, AbstractEventLoop, wait_for
import asyncio
import enum
...
class CLIHandler:
    ...
    def __init__(self, *, db_engine:Engine, debug:bool=False):
        self.debug = debug
    ...
    async def _async_call(self, method:Any, *args, **kwargs) -> Any:
        ...
        self.executor.submit(method, v, *args, **kwargs)
    ...
    async def async_complete_progress_async_action(self, action_id:int, response:Any) -> AsyncActionOpStatus:
        logger.debug(f"CLIHandler:async_complete_progress_async_action: enter, action_id={action_id}")
        r = await self._async_call(self.complete_progress_async_action, action_id, response)
        logger.debug(f"CLIHandler:async_complete_progress_async_action: exit, status={r}")
        return r
    ...
    async def async_wait_for_update_async_action(self, action_id:int, timeout:float) -> Tuple[AsyncActionOpStatus, Optional[AsyncAction]]:
        ...
        if async_action_op_status == AsyncActionOpStatus.DB_FAILED:
            logger.debug(f"CLIHandler:async_wait_for_update_async_action: general db failure")
            logger.debug(f"CLIHandler:async_wait_for_update_async_action: exit, status={async_action_op_status}, async_action=None")
            return (async_action_op_status, None)
    ...
    def start_async_action_unsafe(self, async_call:AsyncCall, request:Any):
        #############################################################
        ...
        try:
            ...
            logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: exit, async_action_op_status={AsyncActionOpStatus.DB_FAILED}, async_action={None}")
            return
    ...
    def start_async_action(self, async_call:AsyncCall, request:Any):
        if self.debug:
            try:
                return self.start_async_action_unsafe(async_call, request)
            except:
    ...
    def update_progress_async_action_unsafe(self, async_call:AsyncCall, action_id:int, progress:Any):
        #############################################################
    ...
    def update_progress_async_action(self, async_call:AsyncCall, action_id:int, progress:Any):
        if self.debug:
            try:
                return self.update_progress_async_action_unsafe(async_call, action_id, progress)
            except:
    ...
    def complete_progress_async_action_unsafe(self, async_call:AsyncCall, action_id:int, response:Any):
        #############################################################
        # Complete an async action's progress
        #############################################################
        logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: enter, action_id={action_id}")
        async_action_info = None
        ...
        try:
            with Session(self.db_engine) as session:
                with session.begin():
                    ...
                    if db_async_action is None:
                        async_call.finish(return_value=AsyncActionOpStatus.NOT_FOUND)
                        logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: exit, status={AsyncActionOpStatus.NOT_FOUND}")
                        return

                    if db_async_action.is_completed:
                        async_call.finish(return_value=AsyncActionOpStatus.ACTION_COMPLETED)
                        logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: exit, status={AsyncActionOpStatus.ACTION_COMPLETED}")
                        return
            ...
            logger.error(f"CLIHandler.complete_progress_async_action_unsafe: unable to update database for async action with id of {action_id}", exc_info=True)
            async_call.finish(return_value=AsyncActionOpStatus.DB_FAILED)
            logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: exit, status={AsyncActionOpStatus.DB_FAILED}")
            return
        ...
        try:
            ...
            for _, client in async_action_info.monitoring_client_dict.items():
                if client.pending_removal:
                    logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: not set event for monitoring client of id {client.id} since it is in pending removal mode")
                else:
                    logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: set event for monitoring client of id {client.id}")
                    client.pending_removal = True
                    client.event.set()

            async_call.finish(return_value=AsyncActionOpStatus.OK)
            logger.debug(f"CLIHandler.complete_progress_async_action_unsafe: exit, status={AsyncActionOpStatus.OK}")
        finally:
            if self.lock.locked():
                self.lock.release()
            if async_action_info is not None and async_action_info.lock.locked():
    ...
    def complete_progress_async_action(self, async_call:AsyncCall, action_id:int, response:Any):
        if self.debug:
            try:
                return self.complete_progress_async_action_unsafe(async_call, action_id, response)
            except:
                logger.error("CLIHandler.complete_progress_async_action_unsafe: exception captured", exc_info=True)
                raise
        else:
            return self.complete_progress_async_action_unsafe(async_call, action_id, response)

    def remove_monitoring_client_unsafe(self, async_call:AsyncCall, action_id:int, moniroting_client_id: uuid.UUID):
        #############################################################
        ...
        try:
            ...
            monitoring_client = async_action_info.monitoring_client_dict.pop(moniroting_client_id)
    ...
    def remove_monitoring_client(self, async_call:AsyncCall, action_id:int, moniroting_client_id: uuid.UUID):
        if self.debug:
            try:
                return self.remove_monitoring_client_unsafe(async_call, action_id, moniroting_client_id)
            except:
    ...
    def register_monitor_async_action_unsafe(self, async_call:AsyncCall, action_id:int):
        #############################################################
        ...
        if self.require_shutdown:
            async_call.finish(return_value=(AsyncActionOpStatus.SHUTDOWN_IN_PROGRESS, None))
            logger.debug(f"CLIHandler.register_monitor_async_action_unsafe: exit, status={AsyncActionOpStatus.SHUTDOWN_IN_PROGRESS}, monitoring_client=None")
            return
    ...
    def register_monitor_async_action(self, async_call:AsyncCall, action_id:int):
        if self.debug:
            try:
                return self.register_monitor_async_action_unsafe(async_call, action_id)
            except:
